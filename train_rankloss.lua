...
if string.len(opt.init_from) > 0 then
  ...
  elseif opt.model == 'gru' then
    ...
    protos.difference = nn.PositiveNegativeDifference()
  elseif opt.model == 'rnn' then
    --protos.rnn = RNN.rnn(vocab_size, opt.rnn_size, opt.num_layers, opt.dropout)
  end
  --protos.criterion = nn.MaskZeroCriterion(nn.ClassNLLCriterion(), 1)
  protos.criterion = nn.SoftMarginCriterion()
end
...
function eval_split(split_index, max_batches)
  ...
  for i = 1,n do -- iterate over batches in the split
    ...
    for t=1,seq_len do
      ...
      clones.difference[t]:evaluate()
      local difference = clones.difference[t]:forward({prediction, y[t]})
      loss = loss + clones.criterion[t]:forward(difference, ones) / seq_len
    end
...
function feval(x)
  ...
  for t=1,seq_len do
    ...
    differences[t] = clones.difference[t]:forward({predictions[t], y[t]})
    loss = loss + clones.criterion[t]:forward(differences[t], ones)
  end
  loss = loss / seq_len
  ------------------ backward pass -------------------
  ...
  for t=seq_len,1,-1 do
    -- backprop decoder
    local doutput_t = clones.criterion[t]:backward(differences[t], ones)
    local ddifference = clones.difference[t]:backward({predictions[t], y[t]}, doutput_t)
    local dprojection = clones.rnn_projection[t]:backward(decoder_rnn_state[t][#init_state], ddifference)
